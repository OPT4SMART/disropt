...
B[dim:dim+NN] = np.ones((NN, 1)) # xi_1 + ... + xi_N = B @ z
D = np.zeros((dim+1+NN, 1))
D[dim] = 1 # b = D @ z
E = np.zeros((dim+1+NN, 1))
E[dim+1+agent_id] = 1 # xi_i = E @ z

obj_func = (1 / 2) * SquaredNorm(A @ z) + C * (B @ z)

# constraints
F = np.zeros((dim+1+NN, NN))
F[dim:dim+NN:, :] = np.eye(NN)

constr = [F @ z >= 0] # non-negativity of slack variables

for j in range(sum(nsamp)):
#     print( float(labels[j]) *( points[:, j].reshape(2,1) @ (A @ z)  + D @ z ) )
    constr.append(float(labels[j]) * ( points[:, j].reshape(2,1) @ (A @ z)  + D @ z ) >= 1 - E @ z) # j-th point
    # TODO: create one xi_{ij} per point instead of a unique \xi_{i} for all the points of the i-th agent
...
# run the algorithm
constrcons_seq = constrcons.run(iterations=n_iter)
# TODO detect real number of iterations

# print results
constrcons_x = constrcons.get_result()

print("Agent {}: {}".format(agent_id, constrcons_x.flatten()))

# save information
np.save("info.npy", {'N': NN, 'size': dim+1, 'iterations': n_iter})
