...
class GTAdamTorch(GradientTrackingTorch):
    def __init__(self, agent: Agent, beta1=0.9, beta2=0.999, epsilon=1e-8, enable_log: bool = False):
        super(GTAdamTorch, self).__init__(agent, enable_log)
        ...
        with torch.no_grad():
            ...
            for p_k in iter(self.x):
                if p_k.requires_grad:
                    direction = np.divide(self.m[k], np.sqrt(self.v[k]) + self.epsilon)
                    p_k.add_(torch.tensor(direction), alpha=-stepsize)
                    k += 1
    ...
    def init_state(self):
        ...
        # initialize momenta
        self.m = [np.zeros_like(e) for e in self.d]
        self.v = [np.zeros_like(e) for e in self.d]
